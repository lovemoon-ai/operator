...
bool GMRRetargeter::configure(const String &scenario, const String &robot_xml,
	...
	try {
		if (s == "whole_body") {
			...
			retargeter_ = retargeting::UpperBodyRetargeter::create(cfg, locked_qpos_prefix);
		} else if (s == "hand") {
...
void GMRRetargeter::_bind_methods() {
	ClassDB::bind_method(D_METHOD("configure", "scenario", "robot_xml", "ik_config", "human_height", "locked_qpos_prefix"),
			&GMRRetargeter::configure, DEFVAL(1.75), DEFVAL(0));
