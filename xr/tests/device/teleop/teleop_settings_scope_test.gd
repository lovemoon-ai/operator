...
func run(_ctx: Dictionary, t: OperatorTestAssertions) -> void:
	...
	if xrt_available:
		...
		var tracking_confirm_button: Button = panel.get("_tracking_confirm_button")
		var confirm_slot: Control = panel.get("_tracking_confirm_slot")
		var robot_tracking_label: Label = panel.get("_tracking_status_label")
		var optional_tracking_label: Label = panel.get("_optional_tracking_status_label")
		t.is_true(robot_tracking_label.text.contains(panel.tr("UI_TRACKING_ROBOT_BODY")), "robot requirement is named explicitly")
		panel.set_tracking_status({"needed": false}, {"needed": true, "mode": "body", "phase": "required", "can_calibrate": true})
		t.is_false(robot_tracking_label.visible, "local body display does not claim the robot requires trackers")
		t.is_true(optional_tracking_label.visible and pico_body_calibration_button.visible, "optional body display retains its own setup entry")
		t.is_true(optional_tracking_label.text.contains(panel.tr("UI_TRACKING_LOCAL_DISPLAY")), "optional tracking is labelled as local display")
		panel.set_tracking_status({"needed": true, "mode": "motion", "phase": "ready", "allowed": true, "can_calibrate": true},
			{"needed": true, "mode": "body", "phase": "mode_conflict"})
		t.is_false(pico_body_calibration_button.disabled, "optional conflict cannot mark ready robot tracking as blocked")
		t.is_true(optional_tracking_label.text.contains(panel.tr("UI_TRACKING_MODE_CONFLICT")), "conflict belongs to optional display")
		panel.set_tracking_status({"needed": true, "mode": "motion", "phase": "mode_conflict", "needs_confirmation": true, "can_confirm": false, "can_calibrate": true},
			{"needed": true, "mode": "body", "phase": "confirmation_waiting_tracking", "needs_confirmation": true, "can_confirm": true})
		t.is_true(pico_body_calibration_button.disabled, "pre-confirmation mode conflict cannot start setup for the wrong mode")
		t.is_true(robot_tracking_label.text.contains(panel.tr("UI_TRACKING_MODE_CONFLICT")), "robot row explains the pre-confirmation conflict")
		panel.set_tracking_status({"needed": false})
		t.is_false(robot_tracking_label.visible or optional_tracking_label.visible or pico_body_calibration_button.visible, "no consumer demand means no tracker setup UI")
		panel.set_tracking_status({"needed": true, "mode": "body", "phase": "required", "can_calibrate": true})
		...
		t.is_true(confirm_slot.visible and tracking_confirm_button.disabled, "return shows confirmation but invalid tracking keeps it disabled")
		...
		t.is_false(tracking_confirm_button.disabled, "live tracking permits explicit user confirmation")
		tracking_confirm_button.pressed.emit()
...
func _test_build_info(panel: TestPanel, t: OperatorTestAssertions) -> void:
	...
	t.eq(group_containers.keys().back(), "build_info", "Teleop sidebar ends with Build info")
	...
	t.contains(build_texts, BuildInfo.version(), "Build info shows the release version")
	t.is_true(BuildInfo.commit().is_valid_hex_number(), "the exported APK carries its source commit")
	t.contains(build_texts, BuildInfo.commit(), "Build info shows the source commit")
