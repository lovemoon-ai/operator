class_name LiveVideoLatencyTracker
extends RefCounted
...
static func format_packet(packet: Dictionary, clock_offset_ns: int = 0, clock_samples: int = 0) -> String:
	...
	# Optional robot->XR clock offset. offset_ns = source_clock - local_clock,
	# so subtract it from source-stamped timestamps to bring them into the
	# local clock domain. A standalone project can omit this and the tx stage
	# will stay hidden until a sync source is supplied.
	...
	if clock_samples > 0 and send_ns_xr > 0 and receive_ns > 0:
	...
	if pipeline_mode == PIPELINE_MODE_FFMPEG:
		...
		if clock_samples > 0 and send_ns_xr > 0 and receive_ns > 0 and receive_ns >= send_ns_xr:
		...
		if total_ns > 0:
			...
			var total_start_ns: int = 0
			if clock_samples > 0:
				total_start_ns = (capture_start_ns - clock_offset_ns) if capture_start_ns > 0 else send_ns_xr
			elif receive_ns > 0:
				total_start_ns = receive_ns
