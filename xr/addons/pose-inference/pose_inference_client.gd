class_name PoseInferenceClient
extends Node
## Full-duplex WebSocket transport for the independent pose-inference mode.
...
func configure_from_qr(payload: String) -> bool:
	...
	if str(config.get("mode", "")) != "pose_inference" or not (url.begins_with("ws://") or url.begins_with("wss://")):
		connection_failed.emit("QR code is not a Pose Inference WebSocket endpoint")
		return false
	_url = url
...
func _format_hand(hand: int) -> Dictionary:
	...
	return {"tracking": _tracking.is_optical_hand_tracking_active(hand), "wrist": _format_pose(wrist), "joints": packed_joints}
...
func _format_pose(value: Dictionary) -> Dictionary:
	...
	return {"tracked": true, "position": [p.x, p.y, p.z], "rotation": [q.x, q.y, q.z, q.w], "radius_m": value.get("radius", 0.0)}
