...
func _on_settings_applied(options: Dictionary) -> void:
	...
	# `_end_video_test()` above can only flip the panel toggle for the *next*
	# read of the form; `options` was captured when Confirm was pressed. OR in
	# the flag so a test that proved video works still reaches the work page
	# even on that path.
