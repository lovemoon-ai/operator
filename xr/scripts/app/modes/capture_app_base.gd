...
func _ready() -> void:
	...
	if automation.has("interaction_mode"):
		var automation_interaction_mode := str(automation["interaction_mode"])
		capture_options["interaction_mode"] = automation_interaction_mode
		...
		# Automation args are parsed after the initial interaction sync in
		# _ready(). Apply the late override now as well, otherwise `hands`
		# changes the manifest label but leaves controller capture enabled.
		_sync_operator_interaction_override()
		_apply_capture_interaction_mode(automation_interaction_mode)
