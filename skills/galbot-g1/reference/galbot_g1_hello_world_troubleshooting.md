...
| 连接 | `ssh root@GALBOT_IP`（SSH 密钥 `~/.ssh/id_ed25519_dang217`） |
...
```bash
ssh root@GALBOT_IP 'cd /data/galbot && ./run_hello.sh hello_world_move_forward.py'
```
...
```
$ ssh root@GALBOT_IP
ssh: connect to host GALBOT_IP port 22: Operation timed out
$ ping -c3 GALBOT_IP   # 100% packet loss
$ nc -z -w5 GALBOT_IP 22 → PORT_22_CLOSED
```
本机 IP `172.16.53.221`，机器人 `GALBOT_IP`，不在同一网段。
...
1. 确认机器人开机并联网。
2. 本机接入正确网络。
3. 复测连通：`ping -c3 GALBOT_IP`，能通再继续。
...
**表现**
```
root@GALBOT_IP: Permission denied (publickey,password).
```
...
1. 配一次免密（会提示输入一次机器人密码）：
   ```bash
   ssh-copy-id -i ~/.ssh/id_ed25519_dang217.pub root@GALBOT_IP
   ```
2. **之后所有 ssh/scp 必须显式指定该 key**：
   ```bash
   ssh -i ~/.ssh/id_ed25519_dang217 root@GALBOT_IP
   ```
...
1. 先建目录，**ssh 命令内联写全**（不要用 zsh 变量拼）：
   ```bash
   ssh -i ~/.ssh/id_ed25519_dang217 root@GALBOT_IP 'mkdir -p /data/galbot/lib'
   ```
2. 用 scp 部署（比依赖远端 rsync 更稳）：
   ```bash
   LIB=galbot_sdk/linux-aarch64-gcc940/lib
   scp -q  -i ~/.ssh/id_ed25519_dang217 "$LIB"/libgalbot_sdk.so*   root@GALBOT_IP:/data/galbot/lib/
   scp -qr -i ~/.ssh/id_ed25519_dang217 "$LIB"/python/galbot_sdk   root@GALBOT_IP:/data/galbot/lib/
   ```
