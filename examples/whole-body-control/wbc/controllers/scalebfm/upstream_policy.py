...
def reconstruct(checkpoint: Path, metadata: dict, mode_table: Path, upstream: Path,
    ...
    for mode, names in ((0, ("pelvis",)),
                        (2, ("pelvis", "left_wrist_yaw_link", "right_wrist_yaw_link")),
                        (4, FIVE_POINTS)):
        if set(torch.nonzero(modes[mode]).flatten().tolist()) != {selected.index(n) for n in names}:
            raise ValueError(f"Unexpected ScaleBFM mode table row {mode}")
...
def deployment_kinematics(path: Path):
    ...
    joints = [i for i in range(1, model.njnt) if "_hand_" not in model.joint(i).name]
    if len(joints) != 29 or model.jnt_type[0] != mujoco.mjtJoint.mjJNT_FREE \
    ...
    bodies = model.jnt_bodyid[[0, *joints]].tolist()
    if len(set(bodies)) != 30 or not np.allclose(model.jnt_pos[joints], 0) \
            or not np.allclose(model.qpos0[model.jnt_qposadr[joints]], 0):
    ...
    return (
        ...
        [model.joint(i).name for i in joints],
        torch.tensor(parents, dtype=torch.long),
        torch.tensor(model.jnt_axis[joints], dtype=torch.float32),
