...
class Simulation:
    ...
    def __init__(self, xml_path: Path, metadata: dict):
        ...
        self.body_joint_names = metadata["joint_names"]
        ...
        if (self.model.nq, self.model.nv, self.model.nu) not in ((36, 35, 29), (50, 49, 43)) \
            ...
            raise ValueError("Use ScaleBridge's floating-base G1 model (29 body + optional 14 hand joints)")
        joint_ids = [self._id(mujoco.mjtObj.mjOBJ_JOINT, n) for n in self.body_joint_names]
        ...
        self.hands = Dex3(self.model) if self.model.nu == 43 else None
        self.reset()
    ...
    def inputs(self, positions: np.ndarray, rotations: np.ndarray, offsets: np.ndarray,
               local_tracking: bool, control_mode: int = 4) -> list[np.ndarray]:
        if control_mode not in (0, 2, 4):
            raise ValueError("ScaleBFM supports root (0), root+wrists (2), or five-point (4) control")
        ...
        return [*history, target_positions.astype(np.float32), target_rotations.astype(np.float32),
                np.array([control_mode], dtype=np.int64), offsets.reshape(1, 6, 1).astype(np.int64)]
    ...
    def blueprint_state(self) -> dict:
        return {
            "g1.joints": self.data.qpos[self.render_indices].tolist(),
