...
from wbc.controllers.scalebfm.runtime import Policy, Simulation
...
@pytest.mark.parametrize("mode,selected", [(0, ("pelvis",)),
    (2, ("pelvis", "left_wrist_yaw_link", "right_wrist_yaw_link"))])
def test_native_root_modes_mask_feet_and_accept_commanded_root_motion(real_policy, mode, selected):
    _, arrays = neutral_inputs(real_policy)
    arrays[7][:] = mode
    baseline = real_policy.infer(arrays)[0]
    changed = [a.copy() for a in arrays]
    unselected = [i for i, name in enumerate(real_policy.metadata["selected_body_names"]) if name not in selected]
    changed[5][:, :, unselected] += .4
    np.testing.assert_allclose(real_policy.infer(changed)[0], baseline, atol=1e-6)
    changed[5][:, :, 0, 0] += .1
    assert not np.allclose(real_policy.infer(changed)[0], baseline, atol=1e-4)


def test_exporter_kinematics_matches_real_mujoco(real_policy):
    import mujoco
    import torch
    from wbc.controllers.scalebfm.upstream_policy import deployment_kinematics, load_definitions
...
def test_real_model_torchscript_export_and_reload(real_policy, tmp_path):
    from wbc.controllers.scalebfm.benchmark import export_script
